...
/**
 * Deterministically selects and shuffles questions using a seed (matchId).
 * Both players in the same match get the exact same questions in the same order.
 */
export function getSeededQuestions(seed: string, count: number = 10): TriviaQuestion[] {
...
/**
 * Strips correctIndex from questions before sending to clients.
 */
export function stripAnswers(questions: TriviaQuestion[]): ClientTriviaQuestion[] {
